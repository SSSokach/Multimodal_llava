...
def load_pretrained_model(model_path, model_base, model_name, load_8bit=False, load_4bit=False, device_map="auto", device="cuda", **kwargs):
    ...
    # ==========================================================================================================
    processor = {'image': None, 'video': None, 'audio': None, 'depth': None, 'thermal': None}
    ...
    if 'llava' in model_name.lower():
        ...
        if mm_use_im_start_end:
            ...
            tokenizer.add_tokens([DEFAULT_THERMAL_START_TOKEN, DEFAULT_THERMAL_END_TOKEN], special_tokens=True)
        model.resize_token_embeddings(len(tokenizer))

        if getattr(model.config, "mm_image_tower", None):
        ...
        if getattr(model.config, "mm_video_tower", None):
        ...
        if getattr(model.config, "mm_depth_tower", None):
            ...
            depth_processor = depth_tower.depth_processor
            processor['depth'] = depth_processor

        if getattr(model.config, "mm_thermal_tower", None):
            thermal_tower = model.get_thermal_tower()
            if not thermal_tower.is_loaded:
                thermal_tower.load_model()
            thermal_tower.to(device=device, dtype=torch.float16)
            thermal_processor = thermal_tower.thermal_processor
            processor['thermal'] = thermal_processor
    # ==========================================================================================================
